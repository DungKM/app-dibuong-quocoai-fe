...
export const RxInbox: React.FC = () => {
  ...
  const handleSelectAll = () => {
      ...
      const allAcks: {[key:string]: boolean} = {};
      selectedItem.changes.forEach(c => {
          if (!c.validationErrors || c.validationErrors.length === 0) {
              allAcks[c.id] = true;
          }
      });
      setAcks(allAcks);
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
            {!selectedItem ? (
                ...
                <>
                    <div className="p-4 sm:p-6 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                             <div className="text-right text-xs text-slate-500 hidden sm:block">
                                 <div>Phiên bản HIS</div>
                                 <div className="font-bold text-lg text-primary">{selectedItem.hisVersionId}</div>
                             </div>
                        </div>
                    </div>
                                    ...
                                    <div key={change.id} className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all relative ${acks[change.id] ? 'border-green-400 ring-1 ring-green-100' : 'border-slate-200 hover:border-blue-300'}`}>
                                        <div className="p-4 grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                                            ...
                                            <div className="md:col-span-1 flex items-center justify-end">
                                                <label className="cursor-pointer p-2 hover:bg-slate-100 rounded-full transition">
                                                    <input 
                                                        type="checkbox" 
                                                        checked={!!acks[change.id]} 
                                                        onChange={() => toggleAck(change.id)}
                                                        className="w-6 h-6 text-green-600 rounded focus:ring-green-500 border-gray-300 cursor-pointer accent-green-600"
                                                    />
                                                </label>
                                            </div>
                                        </div>
                    ...
                    <div className="p-4 sm:p-6 bg-white border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div className="flex items-center gap-4 w-full sm:w-auto">
                            <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-slate-700 hover:text-primary transition select-none">
                                <input type="checkbox" onChange={handleSelectAll} checked={canApply} className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary" />
                                Xác nhận tất cả
                            </label>
                            <span className="text-xs text-slate-400 hidden sm:inline">|</span>
                            <div className="text-xs text-slate-500">
                                Vui lòng kiểm tra kỹ các cảnh báo trước khi áp dụng.
                            </div>
                        </div>
                        
                        <div className="flex gap-3 w-full sm:w-auto">
                            <button className="flex-1 sm:flex-none px-4 py-3 border border-slate-300 bg-white text-slate-700 rounded-lg font-bold hover:bg-slate-50 transition text-sm">
                                Báo sai / Từ chối
                            </button>
                            <button 
                                onClick={() => applyMutation.mutate(selectedItem.patientId)}
                                disabled={applyMutation.isPending || !canApply || selectedItem.changes.some(c => c.validationErrors && c.validationErrors.length > 0)}
                                className="flex-1 sm:flex-none px-6 py-3 bg-primary text-white rounded-lg font-bold shadow-lg shadow-primary/30 hover:bg-sky-600 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
                            >
                                {applyMutation.isPending ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-file-import"></i>}
                                Áp dụng & Cập nhật MAR
                            </button>
                        </div>
                    </div>
                </>
