                ...
                <tr key={user._id || user.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-slate-100 text-slate-400 rounded-xl flex items-center justify-center font-black group-hover:bg-[#1EADED] group-hover:text-white transition-all">
                        {user.fullName?.charAt(0)}
                      </div>
                      <span className="font-bold text-slate-700 uppercase text-sm tracking-tight">{user.fullName}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm font-mono text-slate-400">@{user.username}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase ${
                      user.role === 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-[#1EADED]'
                    }`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-xs font-black text-slate-500 uppercase">{user.idKhoa || "Khoa nhi"}</td>
                  <td className="px-6 py-4">
                    <button 
                      onClick={() => statusMutation.mutate({ id: user._id, active: !user.isActive })}
                      className={`flex items-center gap-2 font-black text-[10px] uppercase transition-all ${
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button className="w-9 h-9 flex items-center justify-center bg-slate-50 text-slate-400 hover:text-primary rounded-xl transition-all">
                        <i className="fa-solid fa-pen-to-square"></i>
                      </button>
                      <button 
                        onClick={() => {
                          const newPass = prompt("Mật khẩu mới:");
                          if(newPass) authApi.resetPassword(user._id, newPass).then(() => alert("Đã reset!"));
                        }}
