...
const MedicationStatsSmall: React.FC<Props> = ({ stats, className }) => {
  if (!stats || stats.total === 0) {
    return (
      <div
        className={`bg-slate-50 text-slate-400 px-3 py-1.5 rounded-xl text-[9px] font-black flex items-center gap-1.5 border border-slate-100 uppercase ${className ?? ""
          }`}
      >
  ...
  if (isDone) {
    return (
      <div
        className={`bg-green-50 text-green-600 px-3 py-1.5 rounded-xl text-[9px] font-black flex items-center gap-1.5 border border-green-100 uppercase ${className ?? ""
          }`}
      >
