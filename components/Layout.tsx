    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around py-2 z-40 pb-[env(safe-area-inset-bottom,20px)] shadow-[0_-1px_10px_rgba(0,0,0,0.05)]">
        ...
        {isAdmin && (
          <Link
            to="/admin/users"
            className={`text-sm font-black px-3 py-2 rounded-md transition flex items-center gap-2 ${location.pathname.startsWith('/admin') ? 'text-purple-600 bg-purple-50' : 'text-slate-600 hover:text-slate-900'}`}
          >
